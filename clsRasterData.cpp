...
template<typename T>
clsRasterData<T>::clsRasterData(string filename, bool getAllValues, clsRasterData<int> *mask) {
    m_rasterPositionData = NULL;
    m_rasterData = NULL;
    m_fileName = filename;
    m_excludeNODATA = getAllValues;
    m_is2DRaster = false;
    m_raster2DData = NULL;
    if (StringMatch(GetUpper(GetSuffix(filename)), ASCIIExtension))
        ReadASCFile(filename, getAllValues, mask);
//    else
...
T clsRasterData<T>::getValue(int row, int col) {
    ...
    if (m_excludeNODATA && m_rasterPositionData != NULL){
...
template<typename T>
void
clsRasterData<T>::outputASCFile(map<string, double> header, int nRows, float **position, T *value, string filename) {
...
//    ofstream rasterFile(filename.c_str());
//    /// Write header
//    rasterFile << nRows << endl;
//    rasterFile << nCols << endl;
//
//    /// Write file
//    int rows = nRows;
//    int cols = nCols;
...
void clsRasterData<T>::ReadASCFile(string ascFileName, bool exludeNodata, clsRasterData<int> *mask) {
    ...
    m_headers[HEADER_RS_CELLSIZE] = tempFloat;
    rasterFile >> tmp >> noData;
    m_headers[HEADER_RS_NODATA] = tempFloat;

    /// if XLLCORNER or YLLCORNER
    if (StringMatch(tmp, "XLLCORNER")) m_headers[HEADER_RS_XLL] -= 0.5 * m_headers[HEADER_RS_CELLSIZE];
    if (StringMatch(tmp, "YLLCORNER")) m_headers[HEADER_RS_YLL] -= 0.5 * m_headers[HEADER_RS_CELLSIZE];
    ...
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            rasterFile >> tempFloat;
            m_rasterData[i * rows + j] = (T)tempFloat;
        }
    }
    rasterFile.close();


    /// if mask data is provided
    if (mask != NULL) {
        /// initial vectors
        vector<T> values;
        vector<int> positionRows;
        ...
        /// if excluding NoData value
        if (m_excludeNODATA)
        {
            /// Get the position data from mask
            int nValidMaskNumber;
            int **validPosition = NULL;
            /// if mask raster data excluding NoData value
            if(mask->excludingNodata()){
                mask->getRasterPositionData(&nValidMaskNumber, &validPosition);
                ...
                for (int i = 0; i < nValidMaskNumber; ++i) {
                    ...
                    double *tmpXY = mask->getCoordinateByRowCol(tmpRow, tmpCol);
                    /// get current raster value by XY
                    int* tmpPosition = this->getPositionByCoordinate(tmpXY[0], tmpXY[1]);
                    T tmpValue = this->getValue(tmpPosition[0], tmpPosition[1]);
                    if (!FloatEqual(tmpValue, noData)){
                        values.push_back(tempFloat);
                ...
                int maskRows = mask->getRows();
                int maskCols = mask->getCols();
                for (int i = 0; i < maskRows; ++i) {
                    for (int j = 0; j < maskCols; ++j) {
                        double *tmpXY = mask->getCoordinateByRowCol(i, j);
                        /// get current raster value by XY
                        int* tmpPosition = this->getPositionByCoordinate(tmpXY[0], tmpXY[1]);
                        T tmpValue = this->getValue(tmpPosition[0], tmpPosition[1]);
                        if (!FloatEqual(tmpValue, noData)){
            ...
            /// create float array
            m_nCells = (int) values.size(); /// this new m_nCells may be different with Mask's m_nCells
            m_rasterData = new T[m_nCells];
            m_rasterPositionData = new int *[m_nCells];
            for (int i = 0; i < m_nCells; ++i) {
                m_rasterData[i] = values.at(i);
            ...
            this->copyHeader(mask->getRasterHeader());
            m_headers[HEADER_RS_NODATA] = noData; /// avoid to assign the nodata of Mask Raster
        } else{  /// Include NoData value
            int maskRows = mask->getRows();
            int maskCols = mask->getCols();
            ...
            /// create new raster data
            if (m_rasterData != NULL) Release1DArray(m_rasterData);
            Initialize1DArray(newRows * newCols, m_rasterData, m_noDataValue);
            for (int k = 0; k < positionRows.size(); ++k) {
                int newIdx = positionRows.at(k) * newRows + positionCols.at(k);
                m_rasterData[newIdx] = values.at(k);
            }

            /// reCalculate header information
            /// Get mask header
            ...
            m_headers[HEADER_RS_NODATA] = double(NODATA_VALUE);
            m_headers[HEADER_RS_CELLSIZE] = (*maskHeader)[HEADER_RS_CELLSIZE];
            m_headers[HEADER_RS_XLL] = (*maskHeader)[HEADER_RS_XLL] + *minCol * m_headers[HEADER_RS_CELLSIZE];
            m_headers[HEADER_RS_YLL] = (*maskHeader)[HEADER_RS_YLL] + (maskRows - *maxRow) * m_headers[HEADER_RS_CELLSIZE];
        ...
        if(m_excludeNODATA) {
            ...
            for (int i = 0; i < rows; ++i) {
                for (int j = 0; j < cols; ++j) {
                    if (FloatEqual(tempFloat, double(noData))) continue;
...
//    m_headers[HEADER_RS_NODATA] = poBand->GetNoDataValue();
...
//#ifndef USE_GDAL
//    ModelException("clsRasterClass", "ReadFromGDAL", "No GDAL library is provided!");
//#else
//    if (mask == NULL)

/*!
 * @brief Test clsRasterData to handle single layer data without calculating position data.
...
class clsRasterDataTestNoPos : public TestWithParam<const char*> {
public:
    clsRasterDataTestNoPos() : rs(nullptr) {}
    ~clsRasterDataTestNoPos() override { delete rs; }
    void SetUp() override {
        rs = new clsRasterData<float>(GetParam(), false);
...
TEST_P(clsRasterDataTestNoPos, RasterIO) {
    ...
    /** Set value **/
    // Set core file name
    string newcorename = corename + "_new_1DNoPos";
...
// factory functions:
INSTANTIATE_TEST_CASE_P(SingleLayerWithoutCalcPosition, clsRasterDataTestNoPos,
